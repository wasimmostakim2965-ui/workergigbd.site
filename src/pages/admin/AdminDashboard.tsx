  ...
  useEffect(() => {
    async function loadStats() {
      ...
      const [
        ...
        supabase.from('deposit_requests').select('amount').eq('status', 'approved').gte('reviewed_at', today.toISOString()),
        supabase.from('withdrawal_requests').select('amount').eq('status', 'approved').gte('reviewed_at', today.toISOString()),

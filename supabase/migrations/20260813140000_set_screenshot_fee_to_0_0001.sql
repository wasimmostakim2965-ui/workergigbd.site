-- ============================================================
-- WorkerGig BD — Set screenshot fee to $0.0001 per screenshot per worker
-- ============================================================
-- Screenshots cost a tiny flat fee of $0.0001 (one hundredth of a cent)
-- per required screenshot, per worker. The job poster prepays it on job
-- creation and the worker receives it on task approval. Deleting a job
-- refunds the unspent portion. This recreates the three money-moving RPCs
-- (post_job, process_task, delete_job) so they charge/pay/refund:
--
--   (base reward + screenshot_count * 0.0001) per worker
--
-- Idempotent. Safe to re-run. Change the SCREENSHOT_FEE constant below
-- to update the fee everywhere.
-- ============================================================
...
BEGIN
  ...
  -- Per-worker cost = base reward + screenshot fee (0.0001 per shot).
...
-- 2. process_task — pay reward + screenshot fee
...
BEGIN
  ...
  -- Payout = base reward + screenshot fee (0.0001 per shot).
  v_reward := (COALESCE(v_task.reward_per_worker,0)
               + COALESCE(v_task.screenshot_count,0) * 0.0001)::numeric(12,3);
...
BEGIN
  ...
  -- Refund the unspent prepaid amount including screenshot fee.

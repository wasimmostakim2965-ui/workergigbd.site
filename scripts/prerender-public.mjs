...
function render([route, title, description, heading, text]) {
  ...
  const body = `<main style="padding:32px 20px;font-family:system-ui,-apple-system,sans-serif;max-width:920px;margin:0 auto"><h1>${escapeHtml(heading)}</h1><p>${escapeHtml(text)}</p><h2>WORKER GIG BD ব্যবহার করার আগে যা জানা দরকার</h2><p>একটি micro-task বেছে নেওয়ার সময় কাজের উদ্দেশ্য, প্রয়োজনীয় steps, reward, completion limit এবং proof requirements আগে পড়ে নিন। কাজটি বাস্তবে সম্পন্ন করার পর নিজের করা কাজের সঠিক proof জমা দিন। Employer হলে এমন instructions লিখুন যাতে workerকে অনুমান করে কাজ করতে না হয় এবং completion criteria সহজে যাচাই করা যায়।</p><p>Marketplace-এর নিয়ম, payment method, withdrawal requirement, membership সুবিধা বা referral reward সময়ের সঙ্গে পরিবর্তিত হতে পারে। তাই পুরোনো পোস্ট বা অন্য কারও দাবি অনুসরণ না করে platform-এর বর্তমান dashboard, task details এবং terms দেখে সিদ্ধান্ত নিন। Account নিরাপত্তার জন্য password, OTP বা login credential কারও সঙ্গে শেয়ার করবেন না।</p><nav aria-label="Related links" style="display:flex;gap:16px;flex-wrap:wrap">${related}</nav></main>`;
  return base
